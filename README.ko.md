...
- [x] 일반적인 번역 시나리오 지원
  ...
  - [x] 선택 번역
    - [x] 어떤 페이지에서든 번역 상자를 열어 여러 서비스 번역 비교
    - [x] 영어 사전
    - [x] 단어 즐겨찾기
  - [x] 마우스 호버 번역
- [x] 다양한 번역 효과
...
- [x] 사용자 정의 단축키
  ...
  - `Alt+C` 번역문 스타일 전환
  - `Alt+O` 설정 페이지 열기
  - 더 많은 단축키는 브라우저 확장 단축키 페이지와 설정 페이지에서 사용자 지정할 수 있습니다
...
- 일부 AI 모델이 통합 번역을 지원하지 않는 경우:
  ...
  - 또는 사용자 정의 인터페이스 방식을 통해 사용합니다. 자세한 내용은 [사용자 정의 인터페이스 예시 문서](custom-api_v2.md)를 참조하세요.
...
- [x] **영어 사전 백업 메커니즘**: 번역 서비스 장애 시 다른 사전으로 전환하거나 로컬 사전으로 폴백.
...
```sh
git clone [https://github.com/xiaochenstarboys/paralleltext.git](https://github.com/xiaochenstarboys/paralleltext.git)
cd paralleltext
pnpm install
pnpm build
```
