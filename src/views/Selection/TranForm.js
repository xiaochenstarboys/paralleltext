...
import { isEnabledEngine } from "../../libs/engineSettings";
...
export default function TranForm({
  ...
  useEffect(() => {
    if (hideConfigBar || !hasUserChangedApiSlugs) {
      setApiSlugs(initApiSlugs);
    }
  }, [initApiSlugs, hasUserChangedApiSlugs, hideConfigBar]);
  ...
  const optApis = useMemo(
    () =>
      transApis.filter(isEnabledEngine).map((api) => ({
        key: api.apiSlug,
  ...
  const activeApiSlugs = useMemo(() => {
    ...
    const preferred = hideConfigBar ? initApiSlugs : apiSlugs;
    const kept = (preferred || []).filter((slug) => validSlugs.has(slug));
                                    
    return kept.length || !preferred?.length
      ? kept
      : optApis.slice(0, 1).map((api) => api.key);
  }, [apiSlugs, initApiSlugs, hideConfigBar, optApis]);
  const translationOperation = useMemo(
    () => createTranslationOperation("selection"),
    [translationText, fromLang, realToLang, activeApiSlugs.join("|")]
  );
  useEffect(() => {
    const close = () => translationOperation.close();
    window.addEventListener("pagehide", close);
    return () => {
      window.removeEventListener("pagehide", close);
      translationOperation.close();
    };
  ...
  const aiDictApiSetting = useMemo(() => {
    ...
    if (!apiSetting || !isEnabledEngine(apiSetting)) {
      return null;
    }
