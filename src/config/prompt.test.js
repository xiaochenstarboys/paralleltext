...
describe("prompt settings", () => {
  test("drops removed engine instances (DeepL/Gemini/Claude) in V3 migration", () => {
    const setting = {
    ...
    expect(migrated.transApis).toHaveLength(1);
    expect(migrated.transApis[0].apiType).toBe(OPT_TRANS_OPENAI);
  ...
  test("cleans api references when a custom prompt is deleted", () => {
    const setting = {
      transApis: [
        {
          apiSlug: "openai",
