...
describe("TransboxManager", () => {
  ...
  test("explicit switch commands are idempotent and retain selection configuration", () => {
    let manager;
    act(() => { manager = new TransboxManager({ tranboxSetting: {
      transOpen: false, apiSlugs: ["Qwen"], triggerMode: "click",
    } }); });
    act(() => manager.toggle(true));
    const host = document.getElementById(APP_CONSTS.boxID);
    act(() => manager.toggle(true));
    expect(document.getElementById(APP_CONSTS.boxID)).toBe(host);
    act(() => manager.update({ tranboxSetting: { transOpen: true } }));
    expect(lastSelectionProps().tranboxSetting).toEqual(expect.objectContaining({ apiSlugs: ["Qwen"], triggerMode: "click" }));
    act(() => { manager.toggle(false); manager.toggle(false); });
