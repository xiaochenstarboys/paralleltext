...
describe("Translator rule styles", () => {
  ...
  test("stopping translation cancels the call chain before backups can start", async () => {
    document.body.innerHTML = '<main id="root"><p>Pending paragraph</p></main>';
    apiTranslate.mockImplementation(() => new Promise(() => {}));
    const translator = createTranslator(
      { apiSlug: "Qwen", autoScan: "false", selector: "p" },
      {
        minLength: 0,
        engineChain: ["Qwen", "DeepSeek"],
        transApis: [
          { ...DEFAULT_API_SETTING, apiSlug: "Qwen", apiType: "Qwen" },
          { ...DEFAULT_API_SETTING, apiSlug: "DeepSeek", apiType: "DeepSeek" },
        ],
      }
    );
    await flushAsync();
  ...
  test("drops a translation identical to the original (misdetected CJK echo)", async () => {
    document.body.innerHTML = '<main id="root"><p>美食</p></main>';
                               
    tryDetectLang.mockResolvedValue("ja");
    apiTranslate.mockResolvedValue({ trText: "美食", isSame: false });

    createTranslator({ fromLang: "auto", toLang: "zh-CN" }, { minLength: 0 });
  ...
  test("drops an identical translation with whitespace differences", async () => {
    document.body.innerHTML = '<main id="root"><p>游戏</p></main>';
    tryDetectLang.mockResolvedValue("ja");
    apiTranslate.mockResolvedValue({ trText: " 游戏\n", isSame: false });
  ...
  test("keeps a translation that actually differs from the original", async () => {
    document.body.innerHTML = '<main id="root"><p>美食</p></main>';
    tryDetectLang.mockResolvedValue("ja");
    ...
    expect(inner.textContent).toBe("美食（特色小吃）");
  });
  ...
  test("protects and restores internal newlines and tabs", async () => {
    ...
    expect(requestedText).not.toContain("\n");
    expect(inner.textContent).toBe("第一\t栏\n第二行");
  });
  ...
  test("keeps literal backslash-t text unchanged", async () => {
    ...
    expect(inner.textContent).toBe("请按字面显示 \\t");
  });
    ...
    document.body.innerHTML = `
      <main id="root">
        <h3>
          ...
          <paralleltext class="paralleltext-wrapper notranslate">
            <font lang="zh-CN" class="paralleltext-inner">Existing translation</font>
          </paralleltext>
        </h3>
      </main>
    `;
      ...
      <main id="root">
        <h3>
          <paralleltext class="paralleltext-wrapper notranslate">
            <br hidden>
            <font lang="zh-CN" class="paralleltext-inner">Existing translation</font>
            <template class="paralleltext-backup">
              <a href="/discussion/1">How to fix playback buttons?</a>
            </template>
          </paralleltext>
        </h3>
  ...
  test("honors persisted plain text rules during initialization", async () => {
    ...
    createTranslator(
      { transOpen: "false", isPlainText: "true" },
      { preInit: true }
    );
    await flushAsync();
  ...
  test("injects inline <style> when CSSStyleSheet constructor is unavailable", async () => {
    ...
    expect(style.id).toBe("paralleltext-fallback-style");
  ...
  test("pairs source and target colors even when a translated sentence splits in two", async () => {
    document.body.innerHTML =
      '<main id="root"><p id="target">First <a href="#example">linked sentence.</a> Second sentence.</p></main>';
    const target = document.getElementById("target");
    const originalHTML = target.innerHTML;
    const firstNode = target.firstChild;
    const link = target.querySelector("a");
    apiTranslate.mockImplementation(async ({ text }) => ({
      trText: text.includes("First") ? "第一部分。还有补充。" : "第二句。",
      isSame: false,
    }));
    const translator = createTranslator(
      {
    ...
    const inner = target.querySelector(`.${Translator.KISS_CLASS.inner}`);
    const translated = inner.querySelectorAll("[data-pt-sentence]");
    expect(translated).toHaveLength(2);
    expect(translated[0].textContent).toBe("第一部分。还有补充。");
    const source = target.querySelectorAll(
      "[data-pt-source-text] [data-pt-sentence]"
    );
    source.forEach((mark) =>
      expect(mark.style.backgroundColor).toBe(
        translated[Number(mark.dataset.ptSentence)].style.backgroundColor
      )
    );
    expect(translated[0].style.backgroundColor).not.toBe(
      translated[1].style.backgroundColor
    );
    translator.updateRule({ transOnly: "true" });
    await flushAsync();
    expect(target.querySelector("[data-pt-source-text]")).toBeNull();
    ...
    expect(apiTranslate).toHaveBeenCalledTimes(2);
    translator.updateRule({ textStyle: "style_none" });
    await flushAsync();
    expect(target.querySelector("[data-pt-source-text]")).toBeNull();
    expect(target.querySelector("a")).toBe(link);
    translator.disable();
  ...
  test("removes source colors when switching styles and ignores a stale sentence request", async () => {
    document.body.innerHTML =
      '<main id="root"><p id="target">First sentence. Second sentence.</p></main>';
    const target = document.getElementById("target");
    const pending = [];
    ...
    const translator = createTranslator(
      { textStyle: "colorful", autoScan: "false", selector: "#target" },
      { minLength: 0 }
    );
    await flushAsync();
    expect(pending).toHaveLength(2);
    apiTranslate.mockResolvedValue({ trText: "整段译文。", isSame: false });
    translator.updateRule({ textStyle: "style_none" });
    await flushAsync();
    pending.forEach((resolve) =>
      resolve({ trText: "迟到译文。", isSame: false })
    );
    await flushAsync();
    expect(target.querySelector("[data-pt-source-text]")).toBeNull();
    expect(
      target.querySelectorAll(`.${Translator.KISS_CLASS.warpper}`)
    ).toHaveLength(1);
    expect(target.textContent).toContain("整段译文。");
    expect(target.textContent).not.toContain("迟到译文。");
    translator.disable();
  });
  ...
  test("paragraphs share a page operation and an explicit new translation gets a new operation", async () => {
    document.body.innerHTML='<div id="root"><p>First paragraph.</p><p>Second paragraph.</p></div>';
    const instance=createTranslator();
    await flushAsync();
    const first=apiTranslate.mock.calls.map(([args])=>args.translationOperation).filter(Boolean);
    expect(first.length).toBeGreaterThanOrEqual(2);
    expect(new Set(first).size).toBe(1);
    apiTranslate.mockClear();instance.disable();instance.enable();await flushAsync();
    const second=apiTranslate.mock.calls.map(([args])=>args.translationOperation).filter(Boolean);
    expect(second.length).toBeGreaterThanOrEqual(2);expect(second[0]).not.toBe(first[0]);
  });
});
